...
// Define constants locally instead of importing from types
...
const ConnectionLine = defineComponent({
  ...
  setup() {
    ...
    const fromNode = computed(() =>
      findNode(connectionStartHandle.value?.nodeId)
    )

    const toNode = computed(
      () => findNode(connectionEndHandle.value?.nodeId) ?? null
    )

    const toXY = computed(() => {
      return {
        x:
          (connectionPosition.value.x - viewport.value.x) / viewport.value.zoom,
        y:
          (connectionPosition.value.y - viewport.value.y) / viewport.value.zoom,
      }
    })

    const markerStart = computed(() =>
      connectionLineOptions.value.markerStart
        ? `url(#${getMarkerId(connectionLineOptions.value.markerStart, id)})`
        : ''
    )

    const markerEnd = computed(() =>
      connectionLineOptions.value.markerEnd
        ? `url(#${getMarkerId(connectionLineOptions.value.markerEnd, id)})`
        : ''
    )
    ...
    return () => {
      ...
      if (toNode.value) {
        ...
        if (connectionMode.value === ConnectionMode.Strict) {
          ...
          toHandle =
            [
              ...(toNode.value.handleBounds.source || []),
              ...(toNode.value.handleBounds.target || []),
            ]?.find((d) => d.id === connectionEndHandle.value?.id) || null
        }
      }

      const toPosition =
        connectionEndHandle.value?.position ??
      ...
      const type =
        connectionLineType.value ??
        connectionLineOptions.value.type ??
