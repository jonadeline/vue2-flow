 ...
 * @param state
 */
export function useViewportHelper(state) {
  function zoom(scale, duration) {
  ...
  function transformViewport(x, y, zoom, duration) {
    return new Promise((resolve) => {
        ...
        for (const node of state.nodes) {
          ...
          if (isVisible) {
            if (
              ...
              (options.nodes?.length && options.nodes.includes(node.id))
            ) {
...
function transition(selection, ms = 0, onEnd) {
