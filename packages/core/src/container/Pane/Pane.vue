...
// Définition locale de SelectionMode au lieu de l'importer
const SelectionMode = {
  Full: 'full',
  Partial: 'partial',
}
...
const hasActiveSelection = computed(
  () =>
    elementsSelectable.value && (props.isSelecting || userSelectionActive.value)
)

const connectionInProgress = computed(
...
<template>
  <div
    ...
    @click="
      (event) =>
        hasActiveSelection ? undefined : wrapHandler(onClick, container)(event)
    "
    @contextmenu="wrapHandler(onContextMenu, container)($event)"
    @wheel.passive="wrapHandler(onWheel, container)($event)"
    @pointerenter="
      (event) => (hasActiveSelection ? undefined : emits.paneMouseEnter(event))
    "
    @pointerdown="
      (event) =>
        hasActiveSelection ? onPointerDown(event) : emits.paneMouseMove(event)
    "
    @pointermove="
      (event) =>
        hasActiveSelection ? onPointerMove(event) : emits.paneMouseMove(event)
    "
    @pointerup="
      (event) => (hasActiveSelection ? onPointerUp(event) : undefined)
    "
    @pointerleave="emits.paneMouseLeave($event)"
  >
    <slot />
    <UserSelection
      v-if="userSelectionActive && userSelectionRect"
      :user-selection-rect="userSelectionRect"
    />
