...
// Define Position constants locally
const Position = {
  Top: 'top',
  Right: 'right',
  Bottom: 'bottom',
...
export function getEdgeHandle(bounds, handleId) {
  if (!bounds) {
    return null
  }
...
export function getEdgeZIndex(edge, findNode, elevateEdgesOnSelect = false) {
  ...
  if (elevateEdgesOnSelect) {
    z = hasZIndex
      ? edge.zIndex
      : Math.max(source.computedPosition.z || 0, target.computedPosition.z || 0)
  }
